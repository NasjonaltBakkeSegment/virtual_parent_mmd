...
def list_one_child(platform, product_type, root_path):
    '''
    Find a single child MMD file for the platform and product_type combination.

    Searches recursively in root_path/platform for XML files that match
    the pattern: platform*product_type*.xml.

    Returns the absolute file path of the first match found, or None if no match is found.
    '''
    search_path = os.path.join(root_path, platform)
    pattern = f"{platform}*{product_type}*.xml"

    return next((os.path.abspath(f) for f in glob.iglob(os.path.join(search_path, '**', pattern), recursive=True)), None)
...
def main(platform, product_type):
    ...
    child_path = list_one_child(platform, filename_product_type, root_path)
